...
/*
    ...
    (BF can still be used to detect negative cycles, but not to find the shortest path in one's presence)
...
vector<int> bellmanFordAdjList(vector<vector<Edge>> &graph, int origin) {
    ...
    /*
        Not that if we want to detect WHAT nodes are [in/affected by] the negative cycle, rather than simply
